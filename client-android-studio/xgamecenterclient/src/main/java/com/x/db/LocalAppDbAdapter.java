...
public class LocalAppDbAdapter {
	...
	public LocalAppDbAdapter(Context context) {

		this.context = context;
		this.dbHelper = new myDBHelper(this.context, LocalAppDbConstant.DB_NAME, null, LocalAppDbConstant.DB_VERSION);
	}
	...
	private void initEntityList() {
		if (entityList == null) {
			if (LocalAppDbConstant.ENTITYS.length > 0) {
				entityList = new ArrayList<EntityBase>();
			}
			ClassLoader cl = myDBHelper.class.getClassLoader();
			for (int i = 0, j = LocalAppDbConstant.ENTITYS.length; i < j; i++) {
				try {
					Object entityobj = cl.loadClass(LocalAppDbConstant.ENTITYS[i].trim()).newInstance();
