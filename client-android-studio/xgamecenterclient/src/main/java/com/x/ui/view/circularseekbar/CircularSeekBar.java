package com.x.ui.view.circularseekbar;
...
/**
* @ClassName: CircularSeekBar
* @Description: Custom CircularSeekBar

* @date 2014-10-17 下午5:47:13
*
 */
public class CircularSeekBar extends View {
	...
	private static final float DEFAULT_POINTER_RADIUS = 3f; // small pointer
	private static final float DEFAULT_POINTER_HALO_WIDTH = 2f; // large pointer
	private static final float DEFAULT_CIRCLE_STROKE_WIDTH = 2f;
	private static final float DEFAULT_POINTER_HALO_BORDER_WIDTH = 2f;
	private static final float DEFAULT_START_ANGLE = 0f; // Geometric (clockwise, relative to 3 o'clock)
	private static final float DEFAULT_END_ANGLE = 180f; // Geometric (clockwise, relative to 3 o'clock)
	...
	private float topCount = 1f;
	private float bottomCount = 1f;
	...
	private void initAttributes(TypedArray attrArray) {
		mCircleXRadius = (float) (attrArray.getFloat(R.styleable.CircularSeekBar_circle_x_radius,
				DEFAULT_CIRCLE_X_RADIUS) * DPTOPX_SCALE);
		mCircleYRadius = (float) (attrArray.getFloat(R.styleable.CircularSeekBar_circle_y_radius,
				DEFAULT_CIRCLE_Y_RADIUS) * DPTOPX_SCALE);
		mPointerRadius = (float) (attrArray
				.getFloat(R.styleable.CircularSeekBar_pointer_radius, DEFAULT_POINTER_RADIUS) * DPTOPX_SCALE);
		mPointerHaloWidth = (float) (attrArray.getFloat(R.styleable.CircularSeekBar_pointer_halo_width,
				DEFAULT_POINTER_HALO_WIDTH) * DPTOPX_SCALE);
		mPointerHaloBorderWidth = (float) (attrArray.getFloat(R.styleable.CircularSeekBar_pointer_halo_border_width,
				DEFAULT_POINTER_HALO_BORDER_WIDTH) * DPTOPX_SCALE);
		mCircleStrokeWidth = (float) (attrArray.getFloat(R.styleable.CircularSeekBar_circle_stroke_width,
		...
		mMax = attrArray.getInt(R.styleable.CircularSeekBar_max_progress, DEFAULT_MAX);
	...
	private void initRects() {
		float padding = context.getResources().getDimension(R.dimen.rectangle_to_circular_padding);
		mCircleRectF.set(-mCircleWidth + padding, -mCircleHeight + padding, mCircleWidth - padding, mCircleHeight
				- padding);
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		paint.setAntiAlias(true); // 消除画笔锯齿
		paint.setColor(Color.WHITE);
		float textSize = ResourceUtil.getDimension(context, R.dimen.circular_progress_text_size);
		paint.setTextSize(textSize);
		canvas.drawText(text, mPointerPositionXY[0] - posX, mPointerPositionXY[1] - posY, paint);
	}
	...
	/*	@Override
		public boolean onTouchEvent(MotionEvent event) {
			...
			case MotionEvent.ACTION_DOWN:
				...
				float pointerRadiusDegrees = (float) ((mPointerRadius * 180) / (Math.PI * Math.max(mCircleHeight,
						mCircleWidth)));
	...
	*/
	private void init(AttributeSet attrs, int defStyle) {
		final TypedArray attrArray = getContext().obtainStyledAttributes(attrs, R.styleable.CircularSeekBar, defStyle,
				0);
	...
	*/
	...
	/**
	* @Title: setTopOffest 
	* @Description: setter position value of top 
	* @param @param progress    
	* @return void
	 */
	public void setTopOffest(int progress) {
		posX = ResourceUtil.getDimension(context, R.dimen.circular_top_margin_posx);
		posY = ResourceUtil.getDimension(context, R.dimen.circular_top_margin_posy);
		if (progress < 10) {
			topCount = 0.5f;
		} else {
			topCount += ResourceUtil.getDimension(context, R.dimen.circular_top_increase_count);
			posX -= topCount;
		}
		text = progress + "%";
	}
	...
	/**
	* @Title: setBottomOffest 
	* @Description: setter position value of bottom 
	* @param @param progress    
	* @return void
	 */
	public void setBottomOffest(int progress) {
		posY = ResourceUtil.getDimension(context, R.dimen.circular_bottom_margin_posy);
		posX = ResourceUtil.getDimension(context, R.dimen.circular_bottom_margin_posx);
		if (progress < 5) {
			bottomCount = 0.5f;
		} else {
			bottomCount += ResourceUtil.getDimension(context, R.dimen.circular_bottom_increase_count);
			posX += bottomCount;
		}
		text = progress + "%";
	}
