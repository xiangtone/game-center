...
/**
 * 
* @ClassName: RingtonesHotFragment
* @Description: 铃声 HOT 界面

* @date 2014-4-8 上午10:47:03
*
 */
public class RingtonesHotFragment extends BaseFragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		adapter = new RingtonesAdapter(mActivity,1,getCt(),"");
	...
	public void onResume() {
		...
		if (currentCountryId == lastCountryId) {
			...
			pulltoRefreshLv.setEmptyView(loadingView);
			adapter.setList(listPath); // 清空adapter，防止显示缓存数据
			getData(1);
		}
	}
	...
	public int getCt() {
		return Constan.Ct.RINGTONES_HOT;
	}
	...
	public void setUserVisibleHint(boolean isVisibleToUser) {
		...
		if (getCt() == Constan.Ct.RINGTONES_HOT) {
			DataEyeManager.getInstance().module(StatisticConstan.ModuleName.RINGTONE_HOT, isVisibleToUser);
			if (isVisibleToUser) {
				DataEyeManager.getInstance().source(StatisticConstan.SrcName.RINGTONE_HOT, 0, null, 0L, null, null,
						false);
			}
		} else if (getCt() == Constan.Ct.RINGTONES_TOP) {
			LogUtil.getLogger().d("test", StatisticConstan.ModuleName.RINGTONE_TOP + "start:" + isVisibleToUser);
			DataEyeManager.getInstance().module(StatisticConstan.ModuleName.RINGTONE_TOP, isVisibleToUser);
			if (isVisibleToUser) {
				DataEyeManager.getInstance().source(StatisticConstan.SrcName.RINGTONE_TOP, 0, null, 0L, null, null,
						false);
			}
		} else if (getCt() == Constan.Ct.RINGTONES_NEW) {
			DataEyeManager.getInstance().module(StatisticConstan.ModuleName.RINGTONE_NEW, isVisibleToUser);
			if (isVisibleToUser) {
