...
/**
 * @ClassName: ListAsGridBaseAdapter
 * @Desciption: listView 嵌套 gridView
 
 * @Date: 2014-3-1 下午3:09:11
 */
public abstract class ListAsGridBaseAdapter extends BaseAdapter {
	...
	public ListAsGridBaseAdapter(Context context, int layoutId) {
		mContext = context;
		mNumColumns = 1;
	...
	private LinearLayout createItemRow(int position, ViewGroup viewGroup, int columnWidth) {
		...
		for (int i = 0; i < mNumColumns; i++) {
			...
			if (currentPos < getItemCount()) {
				...
				insideView = LayoutInflater.from(mContext).inflate(layoutId, null);
