...
public class QuiltViewBase extends GridLayout {
	...
	public boolean isDefDrawable = true;
	...
	public QuiltViewBase(Context context, boolean isVertical,
		...
		this.isDefDrawable = isDefDrawable;
		// 去除一定宽度，使得布局对称
		// int deleteWidth =  getResources().getInteger(R.integer.quiltview_delete_width);
		if (view_width == -1) {
			DisplayMetrics metrics = this.getResources().getDisplayMetrics();
			// int width = metrics.widthPixels - deleteWidth;
	...
	public int getBaseWidth() {
		/*
		 * if(view_width < 500){ columns = 2; } else if(view_width < 801){
		 * columns = 3; } else if(view_width < 1201){ columns = 4; } else
		 * if(view_width < 1601){ columns = 5; } else { columns = 6; }
		 */
		columns = 3; // 目前固定三列
	 ...
	 * MeasureSpec.getSize(heightMeasureSpec); super.onMeasure(widthMeasureSpec,
	 * heightMeasureSpec);
	 * 
	 * view_width = parentWidth; view_height = parentHeight;
	 * 
	 * setup(isVertical); }
	 */
