...
public class FileUtils
{
     ...
     * 程序数据保存的根路径
     */
    public static final String BASE_DIR = "/cs/";
    ...
    public static Intent getStartPackage(String packageName, Context context)
    {
        ...
        {
            final List<ResolveInfo> apps = context.getPackageManager().queryIntentActivities(
                    mainIntent, 0);

            for (ResolveInfo info : apps)
            {
                // Log.v(TAG, " packageName" + info.activityInfo.packageName);
                sMapPackageAndActivity.put(info.activityInfo.applicationInfo.packageName,
    ...
    // public static boolean installApk(DownloadInfo dInfo, Context act) {
    // return installApkWithType(dInfo, act, true);
    // }
    //
    // public static boolean installApkNoInThread(DownloadInfo dInfo, Context
    // act) {
    // return installApkWithType(dInfo, act, false);
    // }
    //
    // /**
    // * 安装下载的文件
    ...
    // * @return
    // */
    // private static boolean installApkWithType(DownloadInfo dInfo, Context
    // act, boolean bInThread) {
    // if (dInfo == null) {
    // return true;
    // }
    //
    // // 统计
    // MainLogicController.Statics.statAddPageCount(PageNo.Action_Install_Apk,
    // dInfo.iProductID, dInfo.iSoftID);
    // MainLogicController.Statics.statAddColumnCount(PageNo.Action_Install_Apk);
    // PackageInfo localPackageInfo =
    // MainLogicController.Apk.getPackageInfoFromLocalApps(dInfo.packageName);
    // if (localPackageInfo != null && localPackageInfo.signatures != null &&
    // localPackageInfo.signatures.length != 0) {
    // if (dInfo.signMD5 != null && dInfo.signMD5.length() != 0) {
    // String strSignature =
    // Tools.MD5(localPackageInfo.signatures[0].toCharsString());
    // if (!strSignature.equalsIgnoreCase(dInfo.signMD5)) {
    // ToastFactory.showToast(R.string.signature_difference);
    // return true;
    // }
    // }
    // }
    // return openFile(dInfo.path, act, dInfo, bInThread);
    // }
    //
    // /**
    // * 卸载
    ...
    //
    // // 统计
    // MainLogicController.Statics.statAddLocalApkCount(JceConstants.LocalAppManagerStatType.Uninstall);
    // }
    //
    // private static synchronized void install(String filePath, File f,
    // DownloadInfo dInfo, Context act) {
    // MainLogicController.Statics.statAddLocalApkCount(LocalAppManagerStatType.Install);
    // String ret = "";
    // if (SettingManager.getInstance().isbShouldInstallInRoot()) {
    // // 尝试用root方式安装
    // ret = RootCommand.runRootCommand("pm install " + filePath);
    // }
    // if (TextUtils.isEmpty(ret) || ret.toLowerCase().indexOf("fail") >= 0) {
    // // 没有安装成功
    // try {
    ...
    // * null
    // */
    // QQMarketApplication.getAppContext().startActivity(intent);
    // } else {
    // ToastFactory.showToast(dInfo.getAppName() +
    // act.getString(R.string.install_success));
    // }
    // }

    // // 在手机上打开文件的method
    // public static boolean openFile(final String filePath, final Context act,
    // final DownloadInfo dInfo, boolean bInThread) {
    // // if( !compareSignature(filePath)){
    // // // 提示签名不一致
    // // Toast.makeText(QQMarketApplication.getAppContext(),
    // // "该安装包与本地软件的签名不一致，请卸载后重新安装!", Toast.LENGTH_SHORT).show();
    // // return true;
    // // }
    ...
    // openResult = false;
    // // 提示安装包已不存在
    // ToastFactory.showToast(R.string.FileNotExist);
    // } else {
    //
    // if (bInThread) {
    // MainLogicController.ThreadTask.postTask(new Runnable() {
    //
    // @Override
    // public void run() {
    // // 统计
    // install(filePath, f, dInfo, act);
    // }
    // });
    // } else {
    // install(filePath, f, dInfo, act);
    ...
    public static void addShortCut(int drawResId, CharSequence name, String packageName,
            Context context)
    {
    ...
    public static int hasShortcutByAppName(Context context, String appName)
    {
        ...
        try
        {
            ...
            c = cr.query(Uri.parse(str), new String[] {
                    "title", "iconResource"
            }, "title=?", new String[] {
                appName
            }, null);
