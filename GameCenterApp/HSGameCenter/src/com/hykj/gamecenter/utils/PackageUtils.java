...
import com.hykj.gamecenter.utils.ShellUtils.CommandResult;
...
public class PackageUtils {
	 ...
	 * @return
	 */
	public static final int install(Context context, String filePath, DownloadTask dInfo) {

		LogUtils.i("install:" + dInfo.appName);

		Log.d(TAG, "Appstore isSystemApplication ? " + PackageUtils.isSystemApplication(context)
				+ " has RootPermission ? " + ShellUtils.checkIsRoot());
		// 检测权限
		if (PackageUtils.isSystemApplication(context) || ShellUtils.checkIsRoot()/*
																				  * ShellUtils
                                                                                  * .
                                                                                  * checkRootPermission
                                                                                  * (
			...
			int ret = installSilent(context, filePath, dInfo);
			// 更改安装完成状态
			DownloadService.getDownloadManager().onInstallEnd(dInfo);
            /*
             * * int ret = ShellUtils.commandHaveRoot(filePath) ?
             * INSTALL_SUCCEEDED : -1;
             */

			LogUtils.e("installSilent ret = " + ret);
			Log.d(TAG, "installSilent ret = " + ret);
			return ret;
			// if (ret == INSTALL_SILENT_SUCCEEDED) {
			// return ret;
			// }

            /*
             * * } else if (ShellUtils.commandHaveRoot(filePath)) { Log.d(TAG,
             * "filePath =" + filePath + "INSTALL_SILENT_SUCCEEDED"); return
             * INSTALL_SILENT_SUCCEEDED; }
             */

		}

		int resule = installNormal(context, filePath) ? INSTALL_SUCCEEDED : INSTALL_FAILED_INVALID_URI;
//        // 更改安装成功状态
		DownloadService.getDownloadManager().onInstallEnd(dInfo);
		return resule;
	}
	...
	public static boolean requestRootPermission() {
		...
		try {
			...
			DataInputStream localDataErrorInputStream = new DataInputStream(p.getErrorStream());
			if (localDataInputStream != null && localDataOutputStream != null) {
				...
				localDataOutputStream.flush();
				// p.waitFor();
				String str1 = localDataErrorInputStream.readLine();
				String str = localDataInputStream.readLine();
				isAllow = !TextUtils.isEmpty(str) && str.contains("uid=0");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
...
//    private static String mCachePath = null;
	public synchronized static boolean installNormal(Context context, String filePath) {
		LogUtils.i("安装应用filePath=" + filePath);
		...
		chmodFile(file);    //提升对应文件的权限


		exceptionNormalInstall(context, filePath);
//        mCachePath = filePath;
	...
	private static void chmodFile(File file) {
		try {
			boolean b = requestRootPermission();
			LogUtils.e("root is b=" + b);
	...
	private static void exceptionNormalInstall(Context context, String filePath) {
	...
	// 常规安装调用
	// public synchronized static boolean installNormal2(Context context, String
	// filePath) {
	// LogUtils.e("常规安装");
	// Log.e(TAG, "常规安装");
	// boolean b = requestRootPermission();
	 ...
	 */
	public static int installSilent(Context context, String filePath, DownloadTask dinfo) {
		...
		StringBuilder command = new StringBuilder().append(
				"LD_LIBRARY_PATH=/vendor/lib:/system/lib:/system/bin pm install -r ").append(
				filePath.replace(" ", "\\ "));
		Log.d(TAG, "installSilent command = " + command);
		...
		if (commandResult.successMsg != null
				...
				.contains("success"))) {
			return INSTALL_SILENT_SUCCEEDED;
		}
	...
	public static Drawable getAppIcon(Context context, String packageName) {
		...
		try {
			app = manager.getApplicationInfo(packageName, 0);
		...
		for (int i = 0; i < packages.size(); i++) {
			...
			if ((packageInfo.applicationInfo.flags & ApplicationInfo.FLAG_SYSTEM) == 0) {
				try {
					if (packageInfo.applicationInfo.processName.equals(app.processName)) {
            ...
            /*
             * if (packageInfo.applicationInfo.equals(app) &&
             * (packageInfo.applicationInfo.flags & ApplicationInfo.FLAG_SYSTEM)
             * == 0) { Logger.i("ConsumeListAdapter", "app==========" +
             * packageInfo.applicationInfo); return
             * packageInfo.applicationInfo.loadIcon(manager);//如果非系统应用 }
             */
		}
		return null;
	}
	 ...
	 * ���ھ�Ĭ��װ
	 */
	public static final int INSTALL_SILENT_PENDING = 10001;
	/**
	 * ��Ĭ��װ�ɹ�
	 */
	public static final int INSTALL_SILENT_SUCCEEDED = 0;
	/**
