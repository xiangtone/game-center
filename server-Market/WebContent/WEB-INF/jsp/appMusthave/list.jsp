...
<body>
	...
	<div class="mainarea_shop">
				<div class="servicesearch pt18">
					<form action="${ctx }/appMusthave/list?type=${type}" method="post">
				...
				<div class="operateShop">
					<ul>
					    <li><a href="${ctx }/appMusthave/add?type=${type}" type="button" class="lang" >添加</a></li>
		...
		<div class="border">
			...
			<div class="page">
				<div class="pageContent">
					<c:if test="${result.recordCount != 0}">
						<ccgk:pagination paginationVo="${result}" contextPath="${ctx}/appMusthave/list" params="${params}"/>
					</c:if>
				</div>
			</div>
		...
		if (id != ''){
			$.alert("删除正常状态记录时，记录会更新为无效，相对应分发数据会删除.\n删除无效状态记录时，记录会从库中删除.\n确定要删除？", function() {
				$.post("delete", {
					...
					window.location.href = "${ctx}/appMusthave/list?type=${type}";
				});
			}, true);
		}
	});
