...
<body>
	...
	<div class="sitemap" id="sitemap">当前位置: <span id="childTitle"></span> -&gt; <font>${country.name}(${country.nameCn})--Wallpaper--Theme</font></div>
		...
		<div class="mainarea_shop">
				<div class="servicesearch pt18">
					<table>
						<tr>
							<td width="400">
								<form id="musicThemeForm" action="${ctx }/wallpaper/listRes/${requestScope.themeId}?resType=theme&raveId=${raveId}" method="post">
							...
							<td width="100">
								<input onclick="window.location.href='${ctx }/imageAlbumTheme/list'" class="bigbutsubmit" type="button" value="返回" >
							</td>
							<td align="left">
								<form action="${ctx }/wallpaper/${requestScope.themeId}/config" method="post">
									<input type="hidden" id="raveId" name="raveId" value="${raveId}"/>																										
				...
				<tbody>
					...
					<c:forEach var="obj" items="${result.data}">
						<tr onMouseOver="chgTrColor(this)" onMouseOut="chgTrColor(this)">
							...
							<td align="center">${obj.imageAlbumTheme.name }(${obj.imageAlbumTheme.nameCn })</td>
				...
				<div class="pageContent">
					<c:if test="${result.recordCount != 0}">
						<ccgk:pagination paginationVo="${result}"
							contextPath="${ctx}/wallpaper/listRes/${requestScope.themeId}"
							params="${params}" />
					</c:if>
				</div>
			</div>
	...
	<script type="text/javascript">
		$(function(){
			var params = {"Q_imageName" : $("#Q_imageName").val(),"resType" : "theme","raveId":"${raveId}"};
