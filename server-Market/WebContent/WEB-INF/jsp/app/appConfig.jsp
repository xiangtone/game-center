...
<body>
	...
	<div class="sitemap" id="sitemap">
		当前位置:<span id="childTitle"></span> -&gt; 自营APP栏目配置
	</div>
			<div class="mainarea_shop">
				<div class="servicesearch pt18">
					<form id="searchForm" action="${ctx }/app/appConfig/${appInfo.id}" method="post">
						...
						<select id="countyId" name="countyId" onchange="change1(this.value)">
							...
							<c:if test="${file.raveId!=1 }">
								<option value="${file.raveId}">${file.nameEn }(${file.raveNames})</option>
							</c:if>
						</select>
						 <input type="button" class="bigbutsubmit" value="提交" id="btn" />
						 <input type="button" class="bigbutsubmit" value="返回" onclick="javascript:window.location.href='${ctx}/app/list';" />
					</form>
				</div>
						...
						<c:forEach items="${result }" var="obj">
								<tr onMouseOver="chgTrColor(this)" onMouseOut="chgTrColor(this)">
									...
									<c:if test="${obj.checked==true }">
										<input type="checkbox" checked="checked" disabled="disabled" readonly="readonly" value="${obj.columnId}"/>
									</c:if> 
	...
	<script type="text/javascript">
					...
					$.ajax({
						...
						success:function(data){
							...
							if(flag.flag == "0"){
								...
								window.document.location = "${ctx }/app/appConfig/${appInfo.id}?apkId=${file.id}&countyId=${raveId}";
							}
						},
						error:function(data){
							alert("配置失败！");
		...
		function change(apkId){
			window.document.location = "${ctx }/app/appConfig/${appInfo.id}?apkId="+apkId;
		}
