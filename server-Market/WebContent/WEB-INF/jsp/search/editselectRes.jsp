...
<body>
	...
	<div class="sitemap" id="sitemap">
		...
		<c:if test="${searchKeyword.albumId==2}">
		&nbsp;&nbsp;<span>App名称(id->Country)注明:(<font color="#912CEE">自营App为紫色,</font><font class="ys1" class="ys1">公用App为绿色,</font>开发者App为黑色)</span>
		</c:if>
	</div>
		...
		<div class="border">
				<table class="mainadd">
					<tbody>
						<tr>
							<td>
								<form id="searchForm" action="${ctx}/search/editSelectRes" method="post">
									...
									<c:if test="${menus0!=null and menus0!=''}">							
									<input type="hidden" name="menus0" value="${menus0}"/>	
									</c:if>	
									...
									<input type="hidden" name="raveId" value="${searchKeyword.raveId}"/>	
									<input type="hidden" name="albumId" value="${searchKeyword.albumId}"/>									
									<input type="hidden" name="flag" value="${searchKeyword.flag}"/>
									<input type="hidden" name="keyword" value="${searchKeyword.keyword}"/>
									<input type="hidden" name="sort" value="${searchKeyword.sort}"/>
						...
						<tr>
							<td colspan="2">
								<form id="editForm"  action="${ctx}/search/show${searchKeyword.searchId}" method="POST">
								<c:if test="${menus1!=null and menus1!=''}">							
								<input type="hidden" name="menus1" value="${menus1}"/>	
								</c:if>	
								<c:if test="${menus0!=null and menus0!=''}">							
									...
									<tr height="55">
										<c:forEach items="${result.data }" var="app" begin="${count }" end="${count + 3 }" step="1" varStatus="app_index">
											...
											<c:if test="${app.appInfo.free!=3 }">
												...
												</td><td><img alt="${app.appName }" src="${path}${app.logo}" width="50" height="50"></td>
											</c:if>
											<c:if test="${searchKeyword.albumId==4}">
												<td width="50">
												<c:choose>
				...
				<div class="page">
					<div class="pageContent">
						<c:if test="${result.recordCount != 0}">
							<ccgk:pagination paginationVo="${result}" contextPath="${ctx}/search/editSelectRes" params="${params}"/>
						</c:if>
					</div>
				</div>
	...
	$(document).ready(function(){
		...
		var params = {"appnameid" : $("#appnameid").val(),"categoryId" : categoryId == "" ? 0 : categoryId,"category_parent" : $("#category_parent").val(),
				"searchId":"${searchKeyword.searchId}","albumId":"${searchKeyword.albumId}","raveId":"${searchKeyword.raveId}",
				"menus0" :"${menus0}","menus1" :"${menus1}","flag" : "${searchKeyword.flag}","keyword" : "${searchKeyword.keyword}",
