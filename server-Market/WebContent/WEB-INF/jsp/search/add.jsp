...
<body>
	...
	<div class="sitemap" id="sitemap">
		当前位置: <a href="list"><span id="childTitle"></span></a> -&gt; 新增搜索关键字
	</div>
	
	<div class="mainoutside mt18">
		<div id="secmainoutside">
			<div class="title">添加搜索关键字</div>
				...
				<table class="mainadd">
					<tbody>
						...
						<tr>
							...
							<td class="content">
							<select name="raveId" id="raveId" onchange="changeCountry(this.value)">
								<c:forEach var="country" items="${countrys}">
									<option value="${country.id}" <c:if test="${searchKeyword!=null and searchKeyword.raveId==country.id}">selected="selected"</c:if> >${country.name}(${country.nameCn})</option>
								</c:forEach>
							</select>&nbsp;&nbsp;
							<span class="red">*</span>
							</td>
						...
						<tr id ="resLogoFileTr" >
							...
							<td>
								<input type="file" class="text" id="resLogoFile" name="resLogoFile" maxlength='150' value="${urlFilePath}"/>
								<span class="red">*</span>&nbsp;&nbsp;<span>请在资源选定后再选择文件</span>
							</td>	
							<td class="content_info">
	...
	$(document).ready(function(){
	    ...
	    $("input[name='flag']").change(function(){
	     ...
	     $("#selectRes1").click(function(){
	        if(flag1 == '1'){
		    	 $("#addForm").attr("action","${ctx }/search/addSelectRes");
		    		$("#addForm").validate({ 
		    			ignore: ".ignore" 
		    			}); 
		    		$("#addForm").submit();
	...
	function changeCountry(value){	
		var raveId = "${searchKeyword.raveId}";
		if($('input[name="deletebtn"]').length!=0){
			 if(confirm("是否要另选资源类别，选择类别时会清除所有以前的数据！")){
				 if(value!=raveId){
					 $("#win").remove(); 
	...
	function addSubmit(){
		...
		if($("#resLogoFile").val()==""&&flag=='1'){
			$.alert('图标不能为空，请上传图标！');
			return;
		}
		$("#addForm").attr("action","${ctx}/search/add");		
