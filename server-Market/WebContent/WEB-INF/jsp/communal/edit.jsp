...
<body>
	...
	<div class="sitemap" id="sitemap">
		当前位置: <a href="list"><span id="childTitle"></span></a> -&gt; 修改app公用信息
	</div>
	
	<div class="mainoutside mt18">
		<div id="secmainoutside">
			<div class="title">修改app公用信息</div>
				...
				<table class="mainadd">
					<tbody>
						...
						<td class="name">应用分类</td>
							<td class="content" colspan="2">
								...
								<c:if test="${category.id<=3}">
									<c:if test="${communal.category.level==1}">
											<option value="${category.id}"  <c:if test="${category.id==communal.category.fatherId }">selected="selected"</c:if> >${category.name}</option>
										</c:if>
										<c:if test="${communal.category.level==2}">
						...
						<tr>
							...
							<td class="content">
							    <input type="hidden" name="id" id="id" value="${communal.id }"/>
								<input type="text" class="text" id="name" name="name"  value="${fn:escapeXml(communal.name) }" maxlength='100' />
								<span class="red">*</span>
							</td>
							<td class="content_info">
						...
						<tr>
							...
							<td class="content">
								<input type="text" class="text" id="source" name="source" value="${fn:escapeXml(communal.source)}" maxlength="50" />
							</td>
						...
						<tr>
							<td class="name">别名</td>
							<td class="content">
								<textarea rows="10" cols="50" id="anotherName" name="anotherName" >${fn:escapeXml(communal.anotherName)}</textarea>
							</td>
							<td class="content_info">
						...
						<tr>
							<td class="name">密码</td>
							<td class="content">
								<input type="text" class="text" id="pwd" name="pwd" value="${fn:escapeXml(communal.pwd)}" maxlength='100'/>
								<input type="hidden" name="free" id="free" value="${fn:escapeXml(communal.free)}" />
							</td>
								<td class="content_info">
						...
						<tr>
							...
							<td colspan="2">
								<textarea rows="1" cols="100" id="bigLogo" name="bigLogo" readonly="readonly">${fn:escapeXml(communal.bigLogo)}</textarea>
							</td>
						</tr>
						<tr>
							...
							<td colspan="2">
								<textarea rows="1" cols="100" id="logo" name="logo" readonly="readonly">${fn:escapeXml(communal.logo)}</textarea>
							</td>
						</tr>
						<tr>
						...
						<tr>
							<td class="name">状态</td>
							<td class="content">
								 <input type="radio" name="state" id="state" value="true" <c:if test="${communal.state==true}">checked="checked"</c:if>/>是
								<input type="radio" name="state" id="state" value="false" <c:if test="${communal.state==false}">checked="checked"</c:if>/>否
							</td>
						</tr>
						 -->
						<tr>
							...
							<td class="content" colspan="2">
								<select name="stars">
									<option value="1" <c:if test="${communal.stars==1}"> selected="selected" </c:if> >1</option>
									<option value="2" <c:if test="${communal.stars==2}"> selected="selected" </c:if>>2</option>
									<option value="3" <c:if test="${communal.stars==3}"> selected="selected" </c:if>>3</option>
									<option value="4" <c:if test="${communal.stars==4}"> selected="selected" </c:if>>4</option>
									<option value="5" <c:if test="${communal.stars==5}"> selected="selected" </c:if>>5</option>
								</select>
							</td>
						</tr>
						<tr>
							...
							<td class="content">
								<textarea rows="10" cols="50" id="brief" name="brief" >${fn:escapeXml(communal.brief)}</textarea>
							</td>
							<td class="content_brief">
						...
						<tr>
							...
							<td>
								<textarea rows="15" cols="100" id="description" name="description">${fn:escapeXml(communal.description)}</textarea>
							</td>
							<td class="content_info">
						...
						<tr>
							...
							<td class="content">
								<input type="text" class="text" id="initDowdload" name="initDowdload" maxlength='10' value="${fn:escapeXml(communal.initDowdload)}" />
							</td>
							<td class="content_info">
						...
						<td>
							<input class="Wdate" id="initialReleaseDate1"  name="initialReleaseDate1" style="width: 160px"
              	 			 ...
              	 			 value="<fmt:formatDate pattern='yyyy-MM-dd' value='${communal.initialReleaseDate}'/>"/>
						</td>
	...
	<script type="text/javascript">
	var level = '${communal.category.level}';
	//当前应用为二级分类
	var firstCat = '${communal.category.fatherId}';
	var secondCat = '${communal.category.id}';
	//当前应用为一级分类
	if(level == '1'){
		firstCat = '${communal.category.id}';
	}
