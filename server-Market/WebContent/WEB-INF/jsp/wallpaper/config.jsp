...
<body>
	...
	<div class="sitemap" id="sitemap">
		...
		<c:if test="${param.resType == 'theme' }"><font>${country.name}(${country.nameCn})--Wallpaper--Theme</font></c:if>
	</div>
		...
		<div class="border">
				<table class="mainadd">
					<tbody>
						<tr>
						<td>
							<form id="searchForm" action="${ctx}/wallpaper/<c:if test="${param.resType == 'theme'}">${param.themeId }</c:if><c:if test="${param.resType != 'theme' }">${requestScope.columnId}</c:if>/config?raveId=${raveId}" method="post">
									Image名字或ImageId：<input type="text" name="imageNameId" id="imageNameId" value="${fn:escapeXml(imageNameId)}" maxlength="100"/>&nbsp;&nbsp;
									请选择: 
								...
								<input onclick="window.location.href='${ctx }/wallpaper/listRes/<c:if test="${param.resType == 'theme'}">${param.themeId }?resType=theme&raveId=${raveId}</c:if><c:if test="${param.resType != 'theme' }">${requestScope.columnId}?raveId=${raveId}</c:if>'" class="bigbutsubmit" type="button" value="返回" >
							</form>
						</td>
						<td class="btn" align="right">
						<form id="editForm" action="${ctx }/wallpaper/<c:if test="${param.resType == 'theme'}">${param.themeId }</c:if><c:if test="${param.resType != 'theme' }">${requestScope.columnId}</c:if>/doConfig" method="post">
							<input type="hidden" id="raveId" name="raveId" value="${raveId}"/>						
							<input type="hidden" name="themeId" value="${param.themeId }">
							<input type="hidden" name="resType" value="${param.resType }">
							<input type="hidden" name="appAlbumId" value="${param.appAlbumId }">
								...
								<c:forEach begin="1" end="${counts}" varStatus="counts_index" step="1" >
									<tr height="55">
										<c:forEach items="${result.data }" var="app" begin="${count }" end="${count + 3 }" step="1" varStatus="app_index">
											<td>
											<table>
											<tr height="55"><td width="50"><input type="checkbox" style="cursor:hand;width:40px;height:40px;" name="menus" value="${app.id}"  /></td><td width="150" >${app.name}(${app.country.name}(${app.country.nameCn}))</td><td><img alt="${app.name }" src="${path}${app.biglogo}" width="50" height="50"></td></tr>
											</table>
											</td>
										</c:forEach>
									</tr>
				...
				<div class="page">
					<div class="pageContent">
						<c:if test="${result.recordCount != 0}">
							...
							<ccgk:pagination paginationVo="${result}" contextPath="${ctx }/wallpaper/${param.themeId }/config" params="${params}"/>
							</c:if>
							<c:if test="${param.resType != 'theme' }">
							<ccgk:pagination paginationVo="${result}" contextPath="${ctx }/wallpaper/${requestScope.columnId}/config" params="${params}"/>
							</c:if>
						</c:if>
					</div>
				</div>
	...
	$(function(){
		...
		var params = {"imageNameId" : $("#imageNameId").val(),
