	...
	<div class="mainoutside">
		<div class="mainarea_shop">
				<div class="servicesearch pt18">
					<form id="searchForm" action="${ctx }/ownApp/list" method="post">
				...
				<div class="operateShop">
					<ul>
					    <li><a href="${ctx }/ownApp/add">添加</a></li>
		...
		<div class="secmainoutside">
					...
					<c:forEach var="obj" items="${result.data}">
						<tr onMouseOver="chgTrColor(this)" onMouseOut="chgTrColor(this)">
							...
							<c:if test="${obj.packageName==null }"> 
							   <a href="${ctx }/ownAppFile/list/${obj.id}"><font color="red">apk管理</font></a>
							</c:if>
							<c:if test="${obj.packageName!=null }"> 
							   <a href="${ctx }/ownAppFile/list/${obj.id}">apk管理</a>
							</c:if>	
							</td>
						</tr>
					</c:forEach>
			...
			<div class="page">
				<div class="pageContent">
					<c:if test="${result.recordCount != 0}">
						<ccgk:pagination paginationVo="${result}" contextPath="${ctx}/ownApp/list" params="${params}"/>
					</c:if>
				</div>
			</div>
