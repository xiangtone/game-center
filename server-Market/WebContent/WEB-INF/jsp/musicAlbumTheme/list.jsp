...
<body>
	...
	<input type="hidden" id="tableName" value="t_res_music_theme"/>
	<div class="mainoutside">
	<div class="mainarea_shop">
				<div class="servicesearch pt18">
					<form action="${ctx }/musicAlbumTheme/list" method="post">
				...
				<div class="operateShop">
					<ul>
					    <li><a href="${ctx }/musicAlbumTheme/add" type="button" class="lang" >添加</a></li>
		...
		<div class="secmainoutside">
				...
				<tbody>
					...
					<c:forEach var="obj" items="${result.data}">
						<tr onMouseOver="chgTrColor(this)" onMouseOut="chgTrColor(this)">
							...
							<td align="center">
						        <c:if test="${obj.state==true}">
							     <a href="${ctx }/ringtones/listRes/${obj.themeId }?resType=theme&raveId=${obj.country.id}">分发</a>
								</c:if>
								<c:if test="${obj.state==false}">
								</c:if>
						</td>
			...
			<div class="page">
				<div class="pageContent">
					<c:if test="${result.recordCount != 0}">
						<ccgk:pagination paginationVo="${result}" contextPath="${ctx}/musicAlbumTheme/list" params="${params}"/>
					</c:if>
				</div>
			</div>
