...
<body>
	<div class="sitemap" id="sitemap">
		当前位置:<a href="${ctx }/market/list" style="color: red;">平台更新管理</a>-&gt;修改APK文件信息
	</div>
	<%-- 初始化弹出窗口 --%>
	...
	<div class="mainoutside mt18">
		<div id="secmainoutside">
			...
			<form id="editForm" action="${ctx }/markAppFile/update/${appFile.appInfo.id}" method="post" enctype="multipart/form-data">
			<div class="border">
				<table class="mainadd">
					<tbody>
							<tr>
						...
						<td class="name">所属渠道(渠道ID)</td>
							<td class="content">
								...
								<c:forEach var="channel" items="${channels}">
									<option value="${channel.id}" <c:if test="${channel.id==appFile.channel.id }">selected="selected"</c:if>>${channel.name}(${channel.id})</option>
								</c:forEach>
								</select>
								<span class="red">*</span>
							</td>
						...
						<tr>
							...
							<td colspan="2">
								<textarea rows="15" cols="50" id="updateInfo" name="updateInfo"  >${fn:escapeXml(appFile.updateInfo) }</textarea>
								 ...
								 <input type="hidden" name="channelId" id="channelId" value="100003">
					 ...
					 success: function(response){
						 ...
						 if(response.flag == "0"){
								$.alert('更新应用成功', function(){
									window.location.href = "${ctx}/markAppFile/list/${appFile.appInfo.id}";
								});
