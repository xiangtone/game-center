...
<body>
				...
				<div class="servicesearch pt18">
					<table>
						<tr>
							<td width="800">
								<form id="applistRes"
								action="${ctx }/ringtones/listRes/${requestScope.columnId}?raveId=${raveId}" method="post">
							...
							<td align="left">
								<form action="${ctx }/ringtones/${requestScope.columnId}/config" method="post">
									<input type="hidden" id="raveId" name="raveId" value="${raveId}"/>																																			
						...
						<tr onMouseOver="chgTrColor(this)" onMouseOut="chgTrColor(this)">
							...
							<c:choose>
								<c:when test="${obj.logo!=null&&obj.logo!=''}">
									<td align="center">
										<img alt="${obj.musicName }" width="50" height="50" src="${path}${obj.logo }">
									</td>
								</c:when>
							<c:otherwise>
								<td align="center">
									<img alt="${obj.musicName }" width="50" height="50" src="${ctx}/static/res/ringtones_disk_pic.png">
								</td>
								</c:otherwise>
							</c:choose>							<td align="center">${obj.stars }</td>
				...
				<div class="pageContent">
					<c:if test="${result.recordCount != 0}">
						<ccgk:pagination paginationVo="${result}"
							contextPath="${ctx}/ringtones/listRes/${requestScope.columnId}"
							params="${params}" />
					</c:if>
				</div>
			</div>
	...
	<script type="text/javascript">
		$(function(){
			var params = {"Q_musicName" : $("#Q_musicName").val(),"raveId":"${raveId}"};
		...
		function selectQuery(id){
			window.location.href = "${ctx }/ringtones/listRes/${requestScope.columnId}?raveId="+id;
		}
	</script>
</body>
</html>