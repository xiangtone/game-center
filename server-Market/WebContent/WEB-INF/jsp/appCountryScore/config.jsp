...
<body>
	...
	<div class="sitemap" id="sitemap">
		当前位置: <span id="childTitle"></span> -&gt; <font>${country.name}(${country.nameCn})--${appAlbumColumn.appAlbum.name}--${appAlbumColumn.name}(${appAlbumColumn.nameCn})</font>
		...
		<div class="border">
				<table class="mainadd">
					<tbody>
						<tr>
						<td>
							<form id="searchForm" action="${ctx}/appCountryScore/config" method="post">
								...
								<input onclick="window.location.href='${ctx }/appCountryScore/list?raveId=${raveId}&albumId=${albumId}&columnId=${columnId}'" class="bigbutsubmit" type="button" value="返回" >
							</form>
						</td>
						<td class="btn" align="right">
							<form id="editForm" action="${ctx }/appCountryScore/doConfig" method="post">
								<input type="hidden" id="raveId" name="raveId" value="${raveId}"/>
								<input type="hidden" id="albumId" name="albumId" value="${albumId}"/>	
								<input type="hidden" id="columnId" name="columnId" value="${columnId}"/>	
								...
								<c:forEach begin="1" end="${counts}" varStatus="counts_index" step="1" >
									<tr height="55">
										<c:forEach items="${result.data }" var="app" begin="${count }" end="${count + 3 }" step="1" varStatus="app_index">
											...
											<tr height="55">
											<td width="50">
											<input type="checkbox" style="cursor:hand;width:40px;height:40px;" name="menus" value="${app.appId}"  />
											</td>
											<td width="150" >
				...
				<div class="page">
					<div class="pageContent">
						<c:if test="${result.recordCount != 0}">
							<ccgk:pagination paginationVo="${result}" contextPath="${ctx }/appCountryScore/config" params="${params}"/>
						</c:if>
					</div>
				</div>
	...
	function querySecondCategory(value){
		if(value == 0){
			...
			$.ajax({
				url : "${ctx}/appCountryScore/secondCategory",
				type : "POST",
				dataType : "json",
