...
<body>
		...
		<div class="border">
				<table class="mainadd">
					<tbody>
						<tr>
						<td>
							<form id="searchForm" action="${ctx}/appAlbumColumn/${appAlbumColumn.columnId}/liveWallpapertoConfig" method="post">
								...
								<input onclick="window.location.href='${ctx }/appAlbumColumn/liveWallpaperListRes/${appAlbumColumn.columnId}?raveId=${raveId}'" class="bigbutsubmit" type="button" value="返回" >
							</form>
						</td>
						<td class="btn" align="right">
						<form id="editForm" action="${ctx }/appAlbumColumn/${appAlbumColumn.columnId}/liveWallpaperDoConfig" method="post">
							<input type="hidden" id="raveId" name="raveId" value="${raveId}"/>	
				...
				<div class="page">
					<div class="pageContent">
						<c:if test="${result.recordCount != 0}">
							<ccgk:pagination paginationVo="${result}" contextPath="${ctx }/appAlbumColumn/${appAlbumColumn.columnId}/liveWallpapertoConfig" params="${params}"/>
						</c:if>
					</div>
				</div>
	...
	$(document).ready(function(){
		var params = {"appnameid" : $("#appnameid").val(),"raveId":"${raveId}"};
