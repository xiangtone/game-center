...
<body>
	...
	<div class="sitemap" id="sitemap">
		当前位置: <span id="childTitle"></span> -&gt; <font>${country.name}(${country.nameCn})--${appAlbumColumn.appAlbum.name }--${appAlbumColumn.name }</font>
		...
		<div class="border">
				<table class="mainadd">
					<tbody>
						<tr>
						<td>
							<form id="searchForm" action="${ctx}/appAlbumColumn/${appAlbumColumn.columnId}/config" method="post">
								...
								<input onclick="window.location.href='${ctx }/appAlbumColumn/handListRes/${appAlbumColumn.columnId}?raveId=${raveId}&temp=${temp}'" class="bigbutsubmit" type="button" value="返回" >
							</form>
						</td>
						<td class="btn" align="right">
						<form id="editForm" action="${ctx }/appAlbumColumn/${appAlbumColumn.columnId}/doConfig" method="post">
							<input type="hidden" id="raveId" name="raveId" value="${raveId}"/>	
							<input type="hidden" id="temp" name="temp" value="${temp }"/>	
							<input type="hidden" name="columnId" id="columnId" value="${appAlbumColumn.columnId}" >
							<input type="hidden" name="albumId" id="albumId" value="${appAlbumColumn.appAlbum.id}" >
				...
				<div class="page">
					<div class="pageContent">
						<c:if test="${result.recordCount != 0}">
							<ccgk:pagination paginationVo="${result}" contextPath="${ctx }/appAlbumColumn/${appAlbumColumn.columnId}/config" params="${params}"/>
						</c:if>
					</div>
				</div>
	...
	$(document).ready(function(){
		...
		var params = {"appnameid" : $("#appnameid").val(),"categoryId" : categoryId == "" ? 0 : categoryId,"category_parent" : $("#category_parent").val(),"raveId":"${raveId}"};
