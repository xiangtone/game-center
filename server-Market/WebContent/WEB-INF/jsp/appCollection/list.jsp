...
<body>
	...
	<input type="hidden" id="tableName" value="t_app_collection"/>
	<input type="hidden" id="type" name="type" value="${type}"/>
	<div class="mainoutside">
	<div class="mainarea_shop">
				<div class="servicesearch pt18">
					<form action="${ctx }/appCollection/list?type=${type}" method="post">
				...
				<div class="operateShop">
					<ul>
					    <li><a href="${ctx }/appCollection/add?type=${type}" type="button" class="lang" >添加</a></li>
		...
		<div class="border">
				...
				<tbody>
					...
					<c:forEach var="obj" items="${result.data}">
						<tr onMouseOver="chgTrColor(this)" onMouseOut="chgTrColor(this)">
							<td align="center"><input type="checkbox" name="recordId" value="${obj.collectionId}"/></td>
							<td align="center">
								<a href="${obj.collectionId }?type=${type}">${obj.collectionId }</a>
							</td>
							<td align="center">
							...
							<td align="center">
								<input type="text" id="sort_${obj.collectionId }" value="${fn:escapeXml(obj.sort) }" maxlength="9" readonly="readonly" style="width:30px;"/>
							</td>
							<td align="center">
							...
							<td align="center">
								<c:if test="${obj.state==true}">
									<a href="${ctx }/appAlbumColumn/collectionRes/${obj.collectionId }?raveId=${obj.country.id}&type=${type}">分发</a>
								</c:if>
								<c:if test="${obj.state==false}">
								</c:if>
						</td>
			...
			<div class="page">
				<div class="pageContent">
					<c:if test="${result.recordCount != 0}">
						<ccgk:pagination paginationVo="${result}" contextPath="${ctx}/appCollection/list" params="${params}"/>
					</c:if>
				</div>
			</div>
	...
	<script type="text/javascript">
	$(function(){
		var params = {"name" : $("#name").val(),"raveId" : $("#raveId").val(),"type" : $("#type").val()};
		...
		if (id != ''){
			$.alert("删除正常状态记录时，记录会更新为无效，相对应分发数据会删除.\n删除无效状态记录时，记录会从库中删除.\n确定要删除？", function() {
				$.post("delete", {
					...
					window.location.href = "${ctx}/appCollection/list?type=${type}";
				});
			}, true);
		}
	});
