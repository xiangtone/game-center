...
<body>
	<div class="sitemap" id="sitemap">
		当前位置:<a href="${ctx }/communal/list" style="color: red;">全部apk管理</a>-&gt;全部apk管理
	</div>
		...
		<div class="mainarea_shop">
				<div class="operateShop">
					...
					<c:if test="${appInfo.free!=3 }">
						<li><a href="${ctx }/communalFile/showAdd/${appInfo.id}">添加</a></li>
					</c:if>
					<li><a id="cmd-delete">删除</a></li>
					<li><a href="${ctx }/communal/list">返回</a></li>
					</ul>
				</div>
			</div>
					...
					<c:forEach var="obj" items="${result}">
						<tr onMouseOver="chgTrColor(this)" onMouseOut="chgTrColor(this)">
							...
							<td align="center" width="100">
								<c:if test="${obj.appInfo.free!=3 }">
									<a href="${ctx }/communalFile/edit/${obj.id }">${obj.id }</a>
								</c:if>
								<c:if test="${obj.appInfo.free==3 }">
									${obj.id }
								</c:if>
							...
							<td align="center">
								<c:if test="${obj.haslist== true }">
									...
									<c:if test="${obj.appInfo.free!=3 }">
										<a href="${ctx }/communalFile/fileList/${obj.id }"  target="_BLANK">查看历史版本</a>
									</c:if>
								</c:if>
