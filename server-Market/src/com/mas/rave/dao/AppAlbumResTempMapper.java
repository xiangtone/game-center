 ...
 * 
 */
public interface AppAlbumResTempMapper {
	 ...
	 * 分页查询app内容信息
	 */
	List<AppAlbumRes> selectByExampleByRaveId(AppAlbumResExample example, RowBounds rowBounds);
	...
	int deleteBySource(AppAlbumRes appAlbumRes);
	 ...
	 * @return
	 */
	int updateRankingByPrimarykey(AppAlbumRes entity);
	...
	void insertOperant(AppAlbumRes appAlbumRes);
	void deleteOperant(AppAlbumRes appAlbumRes);
	void updateOperant(AppAlbumRes appAlbumRes);
	List<AppAlbumRes> getByFileColumnAndSource(AppAlbumRes appAlbumRes);
	List<Integer> getids();
	/**
	 * 根据app信息更新app对应名称
	 * @param appId
	 * @return
	 */
	int upAppAlbumResTempByAppId(HashMap<String, Object> map);
}