<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.Master" AutoEventWireup="true" CodeBehind="F_AppInfoList.aspx.cs" Inherits="AppStore.Web.FindBack.F_AppInfoList" %>
    ...
    <asp:Repeater ID="objRepeater" runat="server">
        ...
        <ItemTemplate>
            <tr class='<%#(Container.ItemIndex%2==0)?"odd":""%>'>
                ...
                <td class="status"><%#Convert.ToInt32(Eval("Status")) == 1 ? "启用" : "禁用" %></td>
                <td><%# this.dic_DevList.ContainsKey(Convert.ToInt32( Eval("CPID")))? this.dic_DevList[Convert.ToInt32(Eval("CPID"))] :"同步中"%></td>
                ...
                <td>
                    <a class="btnSelect" href="javascript:ShareData({AppID:'<%#Eval("AppID") %>', ShowName:'<%#Eval("ShowName")%>', MainIconPicUrl:'<%#Eval("MainIconUrl") %>',TypeID:'<%#Eval("AppType") %>'})"
